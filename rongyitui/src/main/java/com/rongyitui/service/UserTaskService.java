...
public class UserTaskService implements IUserTaskService {
	...
	public Page<UserTask> listUserTask(int type, int pageNo, int pageSize) {
		...
		return this.listUserTask(new TaskHallQuery(type, 0, 0), pageNo, pageSize);
	}

	@Override
	public Page<UserTask> listUserTask(TaskHallQuery query, int pageNo, int pageSize) {
		StringBuffer countHql = new StringBuffer("select count(*) from UserTask as task where 1 = 1");
		StringBuffer pageHql = new StringBuffer("from UserTask as task where 1 = 1");
		...
		if (query != null) {
			...
			if (type != 0) {
				hql.append(" and task.ut_examine = ?");
				list.add(type);
			}
		...
		int count = this.userTaskDao.countByHQL(countHql.toString(), values).intValue();
		if (count == 0) {
			return new Page<UserTask>(pageNo, pageSize, count, Collections.<UserTask> emptyList());
		}
		List<UserTask> pageList = this.userTaskDao.listByHQL(pageHql.toString(), values, pageNo, pageSize);
	...
	public int[] countUserTask() {
		...
		return new int[] { this.countUserTaskByType(0), this.countUserTaskByType(1), this.countUserTaskByType(2),
