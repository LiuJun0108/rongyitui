...
(function(b, c, d) {
	...
	c.extend(c.powerSwitch, {
		...
		transition : function(h, g, f) {
			...
			if (f == true) {
				h.css("webkitTransition", "none").css("transition", "none")
						.data("hasTransition", false)
			} else {
				if (!h.data("hasTransition")) {
					h.css({
						webkitTransition : "-webkit-" + e,
		...
		translate : function(g, e, f) {
			var h = "translate" + e + "(" + f + ")";
			a ? g.css("webkitTransform", h).css("transform", h) : g
					.css(e == "X" ? {
						left : f
					} : {
						top : f
		...
		animation : function(p, h, k) {
			...
			if ((h && h.length) || (p && p.length)) {
				...
				switch (k.animation) {
				case "translate":
					var j = p.data("index"), o = h.data("index");
					var g = {
						vertical : "Y",
						horizontal : "X"
					};
				...
				if (k.container && k.container.length) {
					...
					if (k.direction == "vertical") {
						if (f.scrollHeight - f.clientHeight >= Math.max(m.top,
								1)) {
						...
						if (f.scrollWidth - f.clientWidth >= Math
								.max(m.left, 1)) {
							k.animation == "auto" ? k.container.animate({
								scrollLeft : m.left
	...
	c.fn.powerSwitch = function(k) {
		...
		c
				.each(
						[ "disabled", "prev", "play", "pause", "next" ],
						function(C, F) {
							F = c.trim(F);
							var G = F.slice(0, 1).toUpperCase() + F.slice(1), E = "class"
									+ G, D = z.classPrefix.slice(-1);
							if (z[E] === d) {
								if (z.classPrefix) {
									if (/\-/g.test(z.classPrefix)) {
										z[E] = D == "-" ? (z.classPrefix + F)
												: [ z.classPrefix, F ]
														.join("-")
									} else {
										if (/_/g.test(z.classPrefix)) {
											z[E] = D == "_" ? (z.classPrefix + F)
													: [ z.classPrefix, F ]
															.join("_")
										} else {
											z[E] = z.classPrefix + G
		...
		var m = function(C) {
			if (C <= 0) {
				y.addClass(z.classDisabled).removeAttr("title").attr(
						"disabled", "disabled")
			} else {
				y.removeClass(z.classDisabled).attr("title", y.data("title"))
						.removeAttr("disabled")
			}
			if ((l - C) / h > 1) {
				A.removeClass(z.classDisabled).attr("title", A.data("title"))
						.removeAttr("disabled")
			} else {
				A.addClass(z.classDisabled).removeAttr("title").attr(
						"disabled", "disabled")
			}
		};
		...
		var r = function(G) {
			...
			if (z.toggle == false) {
				...
				if ((z.animation == "visibility" && F.css("visibility") == "hidden")
						|| (z.animation != "visibility" && F.css("display") == "none")) {
					c.powerSwitch.animation(null, F, z);
					display = true
				} else {
							...
							if (z.classDisabled) {
								...
								if (z.container && l > h) {
									if (C == 0) {
										...
										if (F < 0) {
											E = v.eq(B + l);
											c(z.container).data("position",
													E.position());
											c.powerSwitch.animation(null, null,
													c.extend({}, z, {
														animation : "none"
													}));
											F = B + l - h
					...
					c(D).bind(
							...
							function() {
								...
								if (!c(this).attr("disabled")) {
									...
									r.call(this, E);
									if (E >= l - 1) {
										c(this).addClass(z.classDisabled).attr(
												"disabled", "disabled")
												.removeAttr("title")
									}
								}
		...
		if ((x == false && z.toggle == false && t == false)
				|| (t == true && !z.classDisabled)) {
			if (z.container && t == false) {
				var i = "";
				u.length
						&& c.each([ "Prev", "Pause", "Next" ], function(C, D) {
							if (z.autoTime == 0 && D == "Pause") {
								return
							}
							i = i + '<a href="javascript:" class="'
									+ z["class" + D] + '" data-type="'
									+ D.toLowerCase() + '"></a>'
						});
				z.container
						...
						.delegate(
								...
								"click",
								function() {
									var C = c(this).attr("data-type"), E = z["class"
									...
									case "play":
										j.flagAutoPlay = true;
										c(this).attr("data-type", "pause")
												.removeClass(E).addClass(
														z.classPause);
										j();
										break;
									case "pause":
										j.flagAutoPlay = false;
										c(this).attr("data-type", "play")
												.removeClass(E).addClass(
														z.classPlay);
										j();
										break;
			...
			if (z.autoTime) {
				...
				if (t == true
						|| (document.body.contains && z.container && z.container
								.get(0).contains(v.get(0)))) {
					q = [ u, z.container ]
				}
